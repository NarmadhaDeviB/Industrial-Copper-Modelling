...
if selected == "Home":
    ...
    st.subheader(':blue[Overview:] Create a Streamlit app to predict lead status (WON/LOST) and selling price using machine learning models. Incorporate data cleaning, preprocessing, and EDA to prepare your data. The app will let users input data, apply preprocessing, and get predictions for both regression and classification tasks.')
...
if selected == "Selling Price Prediction":
    ...
    if submit_button and flag==0:
            ...
            new_sample_ohe = load_type.transform(new_sample[:, [7]]).toarray()
            new_sample_be = load_status.transform(new_sample[:, [8]]).toarray()
            new_sample = np.concatenate((new_sample[:, [0,1,2, 3, 4, 5, 6,]], new_sample_ohe, new_sample_be), axis=1)
            new_sample1 = load_scalar.transform(new_sample)
...
if selected == "Status Prediction":
    ...
    if csubmit_button and cflag==0:
            ...
            new_sample_ohe = load_ctype.transform(new_sample[:, [8]]).toarray()
            new_sample = np.concatenate((new_sample[:, [0,1,2, 3, 4, 5, 6,7]], new_sample_ohe), axis=1)
            new_sample = load_cscalar.transform(new_sample)
            new_pred = load_cmodel.predict(new_sample)
            if new_pred==1:
